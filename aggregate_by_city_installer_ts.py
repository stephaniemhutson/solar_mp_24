...
def run():
    ...
    late_date = pd.to_datetime('2024-05-31')
    ...
    df_mean = df.groupby(
        ['service_city', 'installer_name', 'year_month', 'service_county']
    )[
        ['size_dc', 'battery_storage', 'total_cost', 'is_largest_firm', 'days_to_completion', 'adjusted_price']
    ].aggregate('mean')

    df_county = df.groupby(
        ['service_city', 'installer_name', 'year_month', 'service_county']
    )[['ccci']].aggregate('max')

    df_count = df.groupby(['service_city', 'installer_name', 'year_month', 'service_county'])[['app_received']].aggregate('count')
    df_count_comp = df.groupby(['service_city', 'installer_name', 'year_month_comp', 'service_county'])[['app_approved']].aggregate('count')

    df_ts = df_mean.join(df_count, on=['service_city', 'installer_name', 'year_month', 'service_county'], how="left")

    df_ts = pd.concat([df_ts, df_count_comp, df_county], axis=1)
    ...
    df_ts = df_ts.join(queue_df, how="left")
    df_ts = df_ts.reset_index()


    # queue_county_df = pd.read_csv('./data/queue_county.csv')
    # queue_county_df['queue'] = queue_county_df['queue'].fillna(0)
    # queue_county_df = queue_county_df.astype(const.TYPE_DICT_QUEUE_COUNTY)[['service_county', 'installer_name', 'date', 'queue']]
    # queue_county_df = queue_county_df.rename({'queue': 'queue_county'})
    # queue_county_df.set_index(['service_county', 'installer_name', 'date'], inplace=True)


    # df_ts.set_index(['service_county', 'installer_name', 'app_received'], inplace=True)
    # df_ts = df_ts.rename(columns={'level_2': 'app_received'})
    # df_ts = pd.concat([df_ts, queue_county_df])
    # df_ts = df_ts.reset_index()
    # df_ts = df_ts[df_ts['total_cost'].notnull()]
